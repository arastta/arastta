...
class ModelFeedFacebookStore extends Model {
	...
	public function getProducts($data = array(), $feeds) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
        ...
        if (!$product_data) {
            foreach ($feeds as $feed) {
                ...
                if ($feed_type[0] == 'product') {
                    ...
                    } else if (in_array($feed_type[0], $special_module) && $module['status']) {
	...
	public function getTotalProducts($data = array(), $feeds) {
		if ($data['filter_category_id'] > 0) {
            $product_data = array();
