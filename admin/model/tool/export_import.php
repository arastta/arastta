...
class ModelToolExportImport extends Model
{
        ...
        foreach ($store_ids as $store_id) {
            ...
            if (!$old_manufacturer) {
                ...
                foreach ($languages as $language) {
                    ...
                    $alias = $this->seo->generateAlias($alias, $manufacturer_id, $language_id);

                    if ($alias) {
                        $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'manufacturer_id=" . (int)$manufacturer_id . "', keyword = '" . $this->db->escape($alias) . "', language_id = '" . (int)$language_id . "'");
                    }
                }
    ...
    protected function storeCategoryIntoDatabase(&$category, &$languages, $exist_meta_title, &$layout_ids, &$available_store_ids, &$url_alias_ids)
    {
        ...
        foreach ($languages as $language) {
            ...
            $seo_keyword = isset($seo_keywords[$language_code]) ? $this->db->escape($seo_keywords[$language_code]) : '';

            if ($seo_keyword) {
                if (isset($url_alias_ids[$category_id])) {
                    $url_alias_id = $url_alias_ids[$category_id];
                    $sql = "INSERT INTO `".DB_PREFIX."url_alias` (`url_alias_id`, `language_id`,`query`,`keyword`) VALUES ($url_alias_id, $language_id,'category_id=$category_id','$seo_keyword');";

                    unset($url_alias_ids[$category_id]);
                } else {
                    $sql = "INSERT INTO `".DB_PREFIX."url_alias` (`language_id`, `query`,`keyword`) VALUES ($language_id, 'category_id=$category_id','$seo_keyword');";
                }
    ...
    protected function uploadCategories(&$reader, $incremental, &$available_category_ids = array())
    {
        ...
        for ($i=0; $i<$k; $i+=1) {
            ...
            while ($this->startsWith($first_row[$j-1], "seo_keyword(")) {
                $language_code = substr($first_row[$j-1], strlen("seo_keyword("), strlen($first_row[$j-1])-strlen("seo_keyword(")-1);

                $seo_keyword = $this->getCell($data, $i, $j++);
                $seo_keyword = htmlspecialchars($seo_keyword);
                $seo_keywords[$language_code] = $seo_keyword;
            }
    ...
    protected function storeProductIntoDatabase(&$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids)
    {
        ...
        foreach ($languages as $language) {
            ...
            $keyword = isset($keywords[$language_code]) ? $this->db->escape($keywords[$language_code]) : '';

            if ($keyword) {
                if (isset($url_alias_ids[$product_id])) {
                    $url_alias_id = $url_alias_ids[$product_id];

                    $sql = "INSERT INTO `".DB_PREFIX."url_alias` (`url_alias_id`, `language_id`,`query`,`keyword`) VALUES ($url_alias_id, $language_id,'product_id=$product_id','$keyword');";
                    unset($url_alias_ids[$product_id]);
                } else {
                    $sql = "INSERT INTO `".DB_PREFIX."url_alias` (`language_id`, `query`,`keyword`) VALUES ($language_id, 'product_id=$product_id','$keyword');";
                }
    ...
    protected function validateCategories(&$reader)
    {
        ...
        if ($exist_meta_title) {
            ...
            $expected_multilingual = array( "name", "seo_keyword", "description", "meta_title", "meta_description", "meta_keywords" );
        } else {
            $expected_heading = array
            ...
            $expected_multilingual = array( "name", "seo_keyword", "description", "meta_description", "meta_keywords" );
        }
    ...
    protected function validateProducts(&$reader)
    {
        ...
        if ($exist_meta_title) {
            $expected_multilingual = array( "name", "seo_keyword", "description", "meta_title", "meta_description", "meta_keywords", "tags", "manufacturer" );
        } else {
            $expected_multilingual = array( "name", "seo_keyword", "description", "meta_description", "meta_keywords", "tags", "manufacturer" );
        }
    ...
    protected function getCategoryKeywords(&$languages, $offset = null, $rows = null, $min_id = null, $max_id = null)
    {
        // query the category_description table for each language
        ...
        foreach ($languages as $language) {
            ...
            $sql  = "SELECT *, SUBSTRING( query, CHAR_LENGTH('category_id=')+1 ) AS category_id FROM `" . DB_PREFIX . "url_alias` ";
            $sql .= "WHERE query LIKE 'category_id=%' AND language_id='".(int)$language_id."' ";

            if (isset($min_id) && isset($max_id)) {
                $sql .= "AND SUBSTRING( query, CHAR_LENGTH('category_id=')+1 ) AS category_id BETWEEN $min_id AND $max_id ";
            }
    ...
    protected function getCategories(&$languages, $exist_meta_title, $offset = null, $rows = null, $min_id = null, $max_id = null)
    {
        ...
        $category_keywords = $this->getCategoryKeywords($languages, $offset, $rows, $min_id, $max_id);
        ...
        foreach ($languages as $language) {
            ...
            foreach ($results->rows as $key => $row) {
                ...
                if (isset($category_keywords[$language_code][$key])) {
                    $results->rows[$key]['keyword'][$language_code] = $category_keywords[$language_code][$key]['keyword'];
                } else {
                    $results->rows[$key]['keyword'][$language_code] = '';
                }
            }
        }
    ...
    protected function populateCategoriesWorksheet(&$worksheet, &$languages, &$box_format, &$text_format, $offset = null, $rows = null, &$min_id = null, &$max_id = null)
    {
        ...
        foreach ($languages as $language) {
            $styles[$j] = &$text_format;
            $data[$j++] = 'seo_keyword('.$language['code'].')';
        }
    ...
    protected function getProductKeywords(&$languages, $offset = null, $rows = null, $min_id = null, $max_id = null)
    {
        ...
        foreach ($languages as $language) {
            ...
            $sql  = "SELECT *, SUBSTRING( query, CHAR_LENGTH('product_id=')+1 ) AS product_id FROM `" . DB_PREFIX . "url_alias` ";
            $sql .= "WHERE query LIKE 'product_id=%' AND language_id='".(int)$language_id."' ";

            if (isset($min_id) && isset($max_id)) {
                $sql .= "AND SUBSTRING( query, CHAR_LENGTH('product_id=')+1 ) AS product_id BETWEEN $min_id AND $max_id ";
            }
    ...
    protected function getProducts(&$languages, $default_language_id, $product_fields, $exist_meta_title, $offset = null, $rows = null, $min_id = null, $max_id = null)
    {
        ...
        $product_keywords = $this->getProductKeywords($languages, $offset, $rows, $min_id, $max_id);
        ...
        foreach ($languages as $language) {
            ...
            foreach ($results->rows as $key => $row) {
                ...
                if (isset($product_keywords[$language_code][$key])) {
                    $results->rows[$key]['keyword'][$language_code] = $product_keywords[$language_code][$key]['keyword'];
                } else {
                    $results->rows[$key]['keyword'][$language_code] = '';
                }
    ...
    public function populateProductsWorksheet(&$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset = null, $rows = null, &$min_id = null, &$max_id = null)
    {
        ...
        foreach ($languages as $language) {
            $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('seo_keyword')+4, 16)+1);
        }
