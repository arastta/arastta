...
class ControllerModuleBlogCategory extends Controller
{
    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if (isset($this->request->post['button']) && $this->request->post['button'] == 'save') {
                $this->response->redirect($this->url->link('module/blog_category', 'token=' . $this->session->data['token'], 'SSL'));
            }
    ...
    protected function getFormFields($data)
    {
        ...
        $form = new AForm('form-blog-category', $action);

        $status = array('value' => $data['blog_category_status'], 'labelclass' => 'radio-inline');
        $form->addElement(new Arastta\Component\Form\Element\YesNo($this->language->get('entry_status'), 'blog_category_status', $status, $option_text));
