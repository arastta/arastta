...
class ClearCacheCommand extends Command {
    ...
    public function fire() {
        ...
        if($this->option('vqcache')) {
            $this->error('vQcache cache clear not setup yet');
        }

        //if no options were passed just remove the application cache
        if(empty(array_filter($this->option())) || $this->option('cache')) {
    ...
    protected function getOptions() {
        return array(
            ...
            array('vqcache', null, InputOption::VALUE_NONE, 'Clear the vQmod cache files'),
